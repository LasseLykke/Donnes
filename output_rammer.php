...
<div class="søge-wrapper">
    ...
    <div class="søge-resultat">
        ...
        //$kundeID = $_SESSION['kundeID']; SKAL MÅSKE MED?
        // Standard SQL-forespørgsel for at hente alle ordre
        $sql = "SELECT ramme.*, kunder.fornavn, kunder.telefonnummer
                FROM ramme
                INNER JOIN kunder ON ramme.kundeID = kunder.kundeID";
        ...
        if (isset($_POST['søg'])) {
            ...
            $sql = "SELECT ramme.*, kunder.fornavn, kunder.telefonnummer
                    FROM ramme
                    INNER JOIN kunder ON ramme.kundeID = kunder.kundeID
                    WHERE kunder.fornavn LIKE '%$search%'
